...
namespace jetreader {
  ...
  Reader::Reader(const std::string &input_file)
    : index_(-1), use_primary_tracks_(true),
      ...
      approx_track_tower_match_(false), is_embedding_(false), manager_(this) {
  ...
  EventStatus Reader::readEvent(size_t idx) {
    ...
    if (centrality_.isValid()) {
      centrality_.setEvent(picoDst()->event()->runId(), picoDst()->event()->refMult(),
  ...
  void Reader::EmbeddingFile(bool flag) {
    is_embedding_ = flag;
  }
  ...
  EventStatus Reader::makeEvent() {
    ...
    // if (!(is_embedding_ && chain()->GetBranchStatus("McTrack")))
    // 	return EventStatus::rejectEvent;
  ...
  bool Reader::selectParticles() {
    bool event_status = true;
    for (int ip = 0; ip < picoDst()->numberOfMcTracks(); ++ip) {
      StPicoMcTrack *part = picoDst()->mcTrack(ip);
      pseudojets_.push_back(MakePseudoJet(*part, part->idVtxStart(), part->idVtxStop()));      
    }
    return event_status;
  }
