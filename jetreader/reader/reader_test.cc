...
TEST(Reader, Load) {
  ...
  EXPECT_EQ(reader.chain()->GetEntries(), 624);
}
...
TEST(Reader, ReadEvent) {
  ...
  EXPECT_EQ(reader.picoDst()->event()->runId(), 15097040);
  EXPECT_EQ(reader.picoDst()->event()->eventId(), 22661);
}
...
TEST(Reader, Next) {
  ...
  EXPECT_EQ(reader.chain()->GetReadEntry(), 623);
}
