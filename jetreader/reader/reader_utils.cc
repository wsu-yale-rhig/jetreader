...
namespace jetreader {
  ...
  fastjet::PseudoJet MakePseudoJet(const StPicoMcTrack &track, int startVtx, int stopVtx) {

    fastjet::PseudoJet j(track.px(), track.py(), track.pz(), track.energy());
    
    VectorInfo *info = new VectorInfo(track, startVtx, stopVtx);
    j.set_user_info(info);
